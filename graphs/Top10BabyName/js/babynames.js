...
var legendGradientBoy = colorLegend.append("defs")
    ...
    .attr("offset", function (d, i) {
        return Math.floor(i / (colorBoys.range().length + 20) * 100) + "%";
    })
...
var legendGradientGirl = colorLegend.append("defs")
    ...
    .enter().append("stop")
    .attr("offset", function (d, i) {
        return Math.floor(i / (colorGirls.range().length + 20) * 100) + "%";
    })
    .attr("stop-color", function (d) {
        return d;
    });
...
strokeLegend.selectAll(".strokeKey")
    ...
    .attr("x", function (d, i) {
        return widthLegend / strokeWidth.length * i;
    })
    .attr("y", function (d, i) {
        return -d / 2;
    })
...
strokeLegend.selectAll(".strokeKeyText")
    ...
    .attr("x", function (d, i) {
        return widthLegend / strokeWidth.length * i + rectWidth / 2;
    })
    .attr("y", 20)
    .style("text-anchor", "middle")
    .text(function (d, i) {
...
var linearGradientBoys = context.selectAll(".linearGradientBoys")
    ...
    .attr("id", function (d) {
        return "line-gradient-boys-" + d.name;
    });
...
linearGradientBoys.append("stop").attr("class", "left").attr("offset", "40%").attr("stop-color", function (d) {
    return colorBoys(d.name);
}).attr("stop-opacity", 1);
linearGradientBoys.append("stop").attr("class", "right").attr("offset", "60%").attr("stop-color", function (d) {
    return colorBoys(d.name);
}).attr("stop-opacity", 1);
...
var linearGradientGirls = context.selectAll(".linearGradientGirls")
    ...
    .attr("x2", xAll(endYear)).attr("y2", "0")
    .attr("id", function (d) {
        return "line-gradient-girls-" + d.name;
    });
...
linearGradientGirls.append("stop").attr("class", "left").attr("offset", "40%").attr("stop-color", function (d) {
    return colorGirls(d.name);
}).attr("stop-opacity", 1);
linearGradientGirls.append("stop").attr("class", "right").attr("offset", "60%").attr("stop-color", function (d) {
    return colorGirls(d.name);
}).attr("stop-opacity", 1);
...
gBrush.selectAll(".resize").append("path")
    ...
    .attr("transform", function (d, i) {
        return i ? "translate(" + -7 + "," + heightAll / 2 + ") rotate(-90)" : "translate(" + 7 + "," + heightAll / 2 + ") rotate(90)";
    });
...
d3.select("#loopStartButton").on("click", function (e) {
    startTimer()
});
d3.select("#loopStopButton").on("click", function (e) {
    stopTimer()
});
...
function searchEvent(name) {
    ...
    if (name != "") {
        ...
        var minYear = Math.max(startYear, d3.min(subset, function (d) {
            return d.year;
        }) - 1);
        var maxYear = Math.min(endYear, d3.max(subset, function (d) {
            return d.year;
        }) + 1);
...
function redraw(choice) {
    ...
    nestedFlatData = d3.nest().key(function (d) {
        return d.name;
    }).entries(flatData);
    ...
    //UPDATE
    contextWrapper.attr("class", function (d) {
        return "focus " + d.name;
    });
    contextWrapper.selectAll(".line")
        ...
        .style("stroke", function (d) {
            return "url(#line-gradient-" + gender + "-" + d.name + ")";
        });
    ...
    //UPDATE
    focusWrapper.attr("class", function (d) {
        return "focus " + d.name;
    });
