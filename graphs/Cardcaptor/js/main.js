...
function create_CCS_chart() {
    ...
    //滚动窗口至文档中的特定位置,https://developer.mozilla.org/zh-CN/docs/Web/API/Window/scroll
    ...
    let size_factor = width / base_width;
    ...
    // SVG 允许我们定义以后需要重复使用的图形元素。 建议把所有需要再次使用的引用元素定义在defs元素里面。这样做可以
    // 增加SVG内容的易读性和可访问性。 在defs元素中定义的图形元素不会直接呈现。 你可以在你的视口的任意地方利用<use>元素呈现这些元素
    ...
    let cover_alpha = 0.3;
    let simulation;
    let remove_text_timer;

    let color_sakura = "#EB5580",
        color_kero = "#F6B42B",
        color_syaoran = "#4fb127";

    //Has a mouseover just happened
    let mouse_over_in_action = false;

    //Radii at which the different parts of the visual should be created
    let rad_card_label = width * 0.4, //capture card text on the outside
    ...
    console.log([pi2 / num_chapters / 2, pi2 + pi2 / num_chapters / 2]);
    ...
    // there are  a lot of groups in the chart.
    // donut group
    // hover-circle-group
    // name-group
    // name-dot-group
    // relation-group
    // relation-hover-group
    // cover-circle-group
    // character-hover-group
    // chapter-group
    // donut-volume-group
    // chapter-hover-group
    // color-group
    // color-circle-hover-group
    // cover-hover-group
    // card-group
    // annotation-group
    // annotations
    ...
    function draw(error, chapter_hierarchy_data, chapter_total_data, character_data, cover_data, character_total_data, relation_data, color_data) {
        ...
        // filter做数据的过滤，返回过滤后新的数据
        chapter_hierarchy_data = chapter_hierarchy_data.filter(function (d) {
            return d.name === "CCS" || (d.volume_num <= num_volume && !d.num) || (d.num >= 1 && d.num <= num_chapters);
        });

        // Based on typical hierarchical clustering example
        // 在计算层次布局之前，你需要一个根节点。如果你的数据已经是层次结构，比如 JSON。你可以直接将其传递给 d3.hierarchy;
        // 此外，你可以重新排列扁平数据，比如将 CSV 使用 d3.stratify 重组为层次结构数据。
        // https://d3js.org.cn/api/d3-hierarchy/#api-referencex
        // https://gerardnico.com/viz/d3/stratify
        // 这里算是对于数据的部分加工，因此在绘图的时候还是需要结合Python工具，将数据做一个基本的格式化
        let root = d3.stratify()
            ...
            //TODO how understand this syntax
            (chapter_hierarchy_data);

        //cluster layout 布局用来生成 dendrograms(系统树)
        //https://d3js.org.cn/api/d3-hierarchy/#hierarchy
        let cluster = d3.cluster()
            //例如如果要生成一个 radial layout(径向布局) 则可以将其设置为 [360, radius] 用以表示角度范围为 360° 并且半径范围为 radius。
        ...
        let chapter_location_data = root.leaves();
        ...
        let chapter_angle_distance = chapter_location_data[1].centerAngle - chapter_location_data[0].centerAngle;
        ...
        // map返回一个数组
        let character_names = character_total_data.map(function (d) {
            return d.character;
        });

        //Sort cover data according to characters from total
        function sortCharacter(a, b) {
            return character_names.indexOf(a.character) - character_names.indexOf(b.character);
        }

        cover_data.sort(sortCharacter);
        character_data.sort(sortCharacter);

        color_data = color_data.filter(function (d) {
            return d.chapter <= num_chapters;
        });
        ...
        //使用指定的 nodes 创建一个新的没有任何 forces(力学模型) 的仿真
        ...
        let image_radius = rad_image;
        let image_group = defs.append("g").attr("class", "image-group");
        ...
        let cover_image = image_group.append("pattern")
            .attr("id", "cover-image")
        ...
        let arc = d3.arc()
            ...
            .padAngle(0.01)
            .cornerRadius((rad_donut_outer - rad_donut_inner) / 2);

        //Pie function to calculate sizes of donut slices, d3.version4
        let pie = d3.pie()
            .sort(null)
            .value(function (d) {
                return d.num_chapters;
            });

        let arcs = pie(character_total_data);
        ...
        let donut_group = chart.append("g").attr("class", "donut-group");
        let slice = donut_group.selectAll(".arc")
            .data(arcs)
            .enter().append("path")
            .attr("class", "arc")
            .attr("d", arc)
        ...
        let name_group = chart.append("g").attr("class", "name-group");

        //Create a group per character
        let names = name_group.selectAll(".name")
            .data(arcs)
            .enter().append("g")
            .attr("class", "name")
            //用于对齐（开始，中间或结束对齐）相对于给定点的文本字符串
        ...
        names.append("text")
            ...
            .attr("transform", function (d, i) {
                ...
                if (character_total_data[i].last_name !== "") {
                    finalAngle = d.centerAngle + (d.centerAngle > 0 && d.centerAngle < Math.PI ? -0.02 : 0.02);
                } else {
                    finalAngle = d.centerAngle;
                }//else
            ...
            .text(function (d, i) {
                return character_total_data[i].first_name;
            });
        ...
        names.append("text")
            ...
            .attr("transform", function (d, i) {
                ...
                if (character_total_data[i].last_name !== "") {
                    finalAngle = d.centerAngle + (d.centerAngle > 0 && d.centerAngle < Math.PI ? 0.03 : -0.03);
                } else {
                    finalAngle = d.centerAngle;
                }//else
                return "rotate(" + (finalAngle * 180 / Math.PI - 90) + ")"
            ...
            .text(function (d, i) {
                return character_total_data[i].last_name;
            });

        //Add one more line for the classmates label
        names.filter(function (d, i) {
            return i === arcs.length - 1;
        })
            .append("text")
            .attr("class", "last-name-label")
            .attr("dy", ".35em")
            .attr("y", "1.35em")
            .attr("transform", function (d, i) {
                let finalAngle = (d.endAngle - d.startAngle) / 2 + d.startAngle - 0.03;
                return "rotate(" + (finalAngle * 180 / Math.PI - 90) + ")"
        ...
        character_total_data.forEach(function (d, i) {
            let text_width_first = document.getElementById('name-label-' + i).getComputedTextLength();
            let text_width_last = document.getElementById('last-name-label-' + i).getComputedTextLength();
        ...
        let hover_circle_group = chart.append("g").attr("class", "hover-circle-group");
        let rad_hover_circle = 35 * size_factor;
        let hover_circle = hover_circle_group.selectAll(".hover-circle")
        ...
        let name_dot_group = chart.append("g").attr("class", "name-dot-group");
        let name_dot = name_dot_group.selectAll(".type-dot")
            .data(character_total_data)
            .enter().append("circle")
            .attr("class", "type-dot")
            .attr("cx", function (d) {
        ...
        let pull_scale = d3.scaleLinear()
            .domain([2 * rad_relation, 0])
            .range([0.7, 2.3]);
        let color_relation = d3.scaleOrdinal()
            .domain(["family", "crush", "love", "friends", "master"]) //"teacher","ex-lovers","reincarnation","rival"
            .range(["#2C9AC6", "#FA88A8", "#E01A25", "#7EB852", "#F6B42B"])
            .unknown("#bbbbbb");
        let stroke_relation = d3.scaleOrdinal()
            .domain(["family", "crush", "love", "friends", "master"]) //"teacher","ex-lovers","reincarnation","rival"
            .range([4, 5, 8, 4, 5])
            .unknown(3);

        let relation_group = chart.append("g").attr("class", "relation-group");

        //Create the lines in between the characters that have some sort of relation
        let relation_lines = relation_group.selectAll(".relation-path")
            .data(relation_data)
            .enter().append("path")
        ...
        function create_relation_lines(d) {
            let source_a = characterByName[d.source].name_angle,
                target_a = characterByName[d.target].name_angle;
            let x1 = rad_relation * Math.cos(source_a - pi1_2),
            ...
            let curve = dr * 1 / pull_scale(dr);

            //Get the angles to determine the optimum sweep flag
            let delta_angle = (target_a - source_a) / Math.PI;
            let sweep_flag = 0;
        ...
        let relation_hover_group = chart.append("g").attr("class", "relation-hover-group");
        let relation_hover_lines = relation_hover_group.selectAll(".relation-hover-path")
            .data(relation_data)
            .enter().append("path")
        ...
        let annotation_relation_group = chart.append("g").attr("class", "annotation-relation-group");
        ...
        function mouse_over_relation(d, i) {
            ...
            let makeAnnotationsRelationship = d3.annotation()
            // .editMode(true)
        ...
        let cover_circle_group = chart.append("g").attr("class", "cover-circle-group");
        let cover_circle = cover_circle_group.append("circle")
        ...
        let arc_character_hover = d3.arc()
            .outerRadius(function (d, i) {
                return character_total_data[i].dot_name_rad + rad_hover_circle;
            })
            .innerRadius(rad_donut_inner);

        //Create the donut slices per character (and the number of chapters they appeared in)
        let character_hover_group = chart.append("g").attr("class", "character-hover-group");
        let character_hover = character_hover_group.selectAll(".character-hover-arc")
            .data(arcs)
            .enter().append("path")
        ...
        function mouse_over_character(d) {
            ...
            ctx.globalAlpha = 0.8;
            create_lines("character", character_data.filter(function (c, j) {
                return c.character === d.character;
            }));
            ...
            clearTimeout(remove_text_timer);
            let label_words = d.character === "Classmates" ? "Naoko, Chiharu, Rika and/or Yamazaki appear" : d.character === "Nakuru" ? "Ruby Moon (also known as Nakuru) appears" : d.character === "Spinel" ? "Spinel Sun appears" : d.character + " appears";
            line_label.text("chapters that " + label_words + " in");

            //Highlight the chapters this character appears in
            let char_chapters = character_data
                .filter(function (c) {
                    return c.character === d.character;
                })
                .map(function (c) {
            ...
            let char_color = characterByName[d.character].color;
            chapter_hover_slice.filter(function (c, j) {
                return _.indexOf(char_chapters, j + 1) >= 0;
            })
                .style("fill", char_color)
                .style("stroke", char_color);
            chapter_number.filter(function (c, j) {
                return _.indexOf(char_chapters, j + 1) >= 0;
            })
                .style("fill", "white");
            chapter_dot.filter(function (c, j) {
                return _.indexOf(char_chapters, j + 1) >= 0;
            })
        ...
        volume_data = volume_data.filter(function (d) {
            return d.volume <= num_volume;
        });
        ...
        var volume_slice = donut_volume_group.selectAll(".volume-arc")
            ...
            .style("stroke", function (d, i) {
                return d.volume <= 6 ? color_kero : color_sakura;
            })
            .style("stroke-width", 3 * size_factor)
        ...
        function mouse_over_chapter(d, i) {
            ...
            ctx.globalAlpha = 1;
            create_lines("chapter", character_data.filter(function (c) {
                return c.chapter === i + 1;
            }));
            ...
            names.filter(function (c) {
                return _.indexOf(char_chapters, c.character) < 0;
            })
                .style("opacity", 0.2);
        ...
        var ccs_colors = color_data.map(function (d) {
                return d.color;
            }),
        ...
        function mouse_over_cover(d, i) {
            ...
            ctx.lineWidth = 4 * size_factor;
            ctx.globalAlpha = 1;
            create_lines("character", cover_data.filter(function (c) {
                return c.chapter === i + 1;
            }));
